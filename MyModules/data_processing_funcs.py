...
def parse_subtable_metastr(one_json_dict, train_special_token_dict):
    ...
    :param one_json_dict:
    :param train_special_token_dict:
    ...
    for item in cell_list:
        cell_text = ' '.join(word_tokenize(
            item.split('<col_header>')[0].strip().split('</cell>')[0].strip().split('<row_header>')[
                0].strip()))
    ...
    if len(one_sub_str.split('<section_title>')) > 1:
        section_title_text = ' '.join(word_tokenize(
            one_sub_str.split('<section_title>')[1].split('</section_title>')[0].strip()))
    else:
        section_title_text = ''
...
def separate_parse_snippet(text, tokenizer, is_front):
    ...
    for idx in range(len(token_list)):
        if idx == 0:
            ...
            one_token = ' ' + token_list[idx]
        one_token_id = tokenizer.encode(one_token, max_length=512, truncation=True,
...
def join_cell_list_and_order_map(ordered_cell_list, map_dict, tokenizer, special_token_dict):
    ...
    for item in ordered_cell_list:
        ...
        one_len = len(
            tokenizer.encode(one_text, max_length=512, truncation=True, add_special_tokens=False))
    ...
    assert all_src_id_list == tokenizer.encode(input_text, max_length=512, truncation=True,
                                               add_special_tokens=False)
